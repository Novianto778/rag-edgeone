...
export default function CitationCard({
  ...
  const formatPageLabel = (pageNum: number) => {
    if (lang === "zh") {
        ...
        {content && content.length > 0 && (
          <div className="citation-pages">
            {content.map((item, idx) => {
              ...
              const canExpand = item.content && item.content.length > 400;

              return (
                <div key={idx} className="citation-page-item">
                  <div className="citation-page-header">
                    <span className="citation-page-number">
                      {formatPageLabel(item.page)} {item.sectionPath ? `• ${item.sectionPath}` : ''}
                    </span>
